...
if(isset($_POST['duration']) && $_POST['duration'])
{
    ...
    $stmt = $conn->prepare("SELECT id,name,type,size,createdon FROM playlists WHERE userid=$user ORDER BY createdon DESC");
            ...
            { 
                ...
                if(sizeof($examids)>0){
                    foreach ($examids as $examid) //fetching the contents of the row
                    {
                        ...
                        //getting examdetails
                        $stmt2=$conn->prepare("SELECT id,name,type,size,createdon FROM playlists WHERE id=?");
