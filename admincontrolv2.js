...
function searchexams(diff){
   $.ajax({
       type: "POST",
       url: 'myplaylist_page_handle.php',
       data: {
           duration:diff
       },
       success: function(response)
       {
           ...
           if (message.success == "1")
           {
               ...
               while(count<jsonData.length) {
                   ...
                   table.append(
                        `<div class="row " >            <div class="col-2 cell" id="examid">${element.id}</div>            <div class="col-4 cell" id="examtitle">${element.title}</div>            <div class="col-2 cell" id="examdate">${element.date}</div>            <div class="col-1 cell" id="examtime">${element.type}</div>            <div class="col-1 cell"><button class="btn btn-outline-success viewbtn"  value='${element.title}'><i class="far fa-eye"></i></button></div>         <div class="col-1 cell"><button class="btn btn-outline-danger deletebtn"  value='${element.id}'><i class="far fa-trash-alt"></i></button></div>    </div>`    
                       );              
               count++;
               };
...
$('#table').on('click','.deletebtn',function(){
   if(confirm("Are you sure you want to delete this Exam."))
   {
       $.ajax({
           type: "POST",
           url: 'deleteexam_byteacher_handlev2.php',
           data: {
               examid:$(this).val(),
               from:"delete"
           },
